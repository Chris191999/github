...
def show_trade_form():
    ...
    with st.form(key="trade_form"):
        ...
        with pos_col1:
            ...
            pnl = st.number_input("Profit/Loss ($)", 
        ...
        if direction == "Long" and entry > 0 and stop_loss > 0:
            risk_per_unit = entry - stop_loss
        elif direction == "Short" and entry > 0 and stop_loss > 0:
            risk_per_unit = stop_loss - entry
        else:
            risk_per_unit = 0
            
        if direction == "Long" and entry > 0 and take_profit > 0:
            reward_per_unit = take_profit - entry
        elif direction == "Short" and entry > 0 and take_profit > 0:
            reward_per_unit = entry - take_profit
        else:
            reward_per_unit = 0
            
        # Initialize calculated_rrr
        calculated_rrr = 0.0
        
        if risk_per_unit > 0 and reward_per_unit > 0:
            calculated_rrr = reward_per_unit / risk_per_unit
            st.info(f"Calculated Risk-Reward Ratio: {calculated_rrr:.2f}")
        
        # Hidden fields to keep compatibility
        planned_rrr = calculated_rrr if risk_per_unit > 0 and reward_per_unit > 0 else 0.0
        actual_rrr = calculated_rrr if status == "Completed" else 0.0
